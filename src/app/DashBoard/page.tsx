...
const DashBoard = () => {
    ...
    const device: { name: string, mac: string, ip: string, created: string, powerConsumption: number }[] = [{
        name: "TV",
        ip: "172.0.2",
    ...
    const handleAddDevice = () => {
        if (validateDevice(deviceName, deviceIp)) {
            const deviceValue = dataDevice.filter((filterDevice) => filterDevice.ip === deviceIp);
            if (deviceValue.length < 1) {
                setDataDevice([...dataDevice, { name: deviceName, mac: generateRandomMacAddress(), ip: deviceIp, created: formatDate(), powerConsumption: generateRandomPower() }])
            } else {
                setDataDevice([...dataDevice, { name: deviceName, mac: generateRandomMacAddress(), ip: deviceValue[0].ip, created: formatDate(), powerConsumption: deviceValue[0].powerConsumption }])
            }
            setDeviceName("");
            setDeviceIP("");
    ...
    const getTotal = useMemo(() => {
        const result = dataDevice.reduce((total, index) => { return total + index.powerConsumption; }, 0)
        return result
    }, [dataDevice]);
                    ...
                    <div className="table">
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr className="totalPower"><td>Total</td> <td></td> <td></td> <td></td> <td>{getTotal}</td> </tr>
                            </tbody>
                    ...
                    <div className="chart">
                        ...
                        <div className="chart--form">
                            ...
                            <div className="chart--form__submit">
                                <button className="btn" onClick={handleAddDevice}>ADD DEVICE</button>
                            </div>
                        </div>
                    </div>
                </div>
